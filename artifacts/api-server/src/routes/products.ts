...
import { authMiddleware, requireSupplier, type AuthRequest } from "../middlewares/auth";
...
router.get("/products", async (req, res): Promise<void> => {
  const { search, categoryId, supplierId, available, page = "1", limit = "20" } = req.query;
  ...
  if (search) {
    query = query.where(ilike(productsTable.nome, `%${search}%`)) as typeof query;
  } else if (conditions.length > 0) {
    query = query.where(and(...conditions)) as typeof query;
  }

  const products = await query.limit(limitNum).offset(offset).orderBy(asc(productsTable.createdAt));
