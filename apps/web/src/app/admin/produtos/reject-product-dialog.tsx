...
import { rejectProductAction } from "@/server/actions/product-moderation-actions";
...
function SubmitButton() {
  ...
  return (
    <Button type="submit" variant="destructive" disabled={pending}>
      {pending ? "Rejeitando..." : "Rejeitar produto"}
    </Button>
  );
}

export function RejectProductDialog({ productId, productName }: { productId: string; productName: string }) {
  const [open, setOpen] = useState(false);
  const action = rejectProductAction.bind(null, productId);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Rejeitar &ldquo;{productName}&rdquo;</DialogTitle>
        </DialogHeader>
        <form action={formAction} className="space-y-4" noValidate>
          <div className="space-y-2">
            <Label htmlFor="reason">Motivo da rejeição</Label>
            <Textarea id="reason" name="reason" required rows={4} placeholder="Explique o que precisa ser corrigido..." />
